...
before(async () => {
    ...
    const loginResult = await api.post('/api/login').send({username: initialUser.username, password: initialUser.password})
...
describe('Deleting of blogs', () => {
    ...
    test('Checking if deleting content works', async () => {
        ...
        await api.del(`/api/blogs/${dbBlogs[0].id}`).set('Authorization', `Bearer ${token}`).expect(204)
    })

    test('Checking if deleting content removes from the database', async () => {
        const result = await api.get('/api/blogs')
        const dbBlogs = result.body
        await api.del(`/api/blogs/${dbBlogs[0].id}`).set('Authorization', `Bearer ${token}`)
...
describe('Input testing', () => {
    ...
    test('Likes property defaults to 0 if it isnt provided', async () => {
        const response = await api.post('/api/blogs').set('Authorization', `Bearer ${token}`).send(missingLikesBlog)
